...
public class IUImprimirDatosPersona extends IUVentanaT{
    ...
    private void construirPaneles(Limitacion limite){
        ...
        etiquetaTitulo = new IUEtiqueta("Panel Tamaño Carta", new Limitacion(limite.getPorcentajeAncho(1), limite.getPorcentajeAlto(2), limite.getPorcentajeAncho(62), limite.getPorcentajeAlto(4)));
        etiquetaTitulo.setHorizontalAlignment(SwingConstants.CENTER);
        etiquetaTitulo.setFont(new Font("Verdana", Font.PLAIN, limite.getPorcentajeAlto(3)));
    ...
    private void escucharEvento(){
        ...
        botonImprimir.addEventoRaton(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                Component[] arregloComenponentes = panelHojaImpresion.getComponents();
                if(arregloComenponentes.length > 0 ){
                    for (int i = 0; i < arregloComenponentes.length; i++) {
                        Component componente = arregloComenponentes[i];
                        if(componente.isVisible())
                            componentes.add(componente);
                    }
                    if(!componentes.isEmpty()){
                        ImprimirPanel imprimirDocumentos = new ImprimirPanel(componentes, panelHojaImpresion);
                    }                        
                }else{
                    Asistente.mensajeVerificacion(ventanaPrincipal, "aviso", "lo siento... pero no existe algun documento para imprimir.", "advertencia");
                }
            }
        });
    }
